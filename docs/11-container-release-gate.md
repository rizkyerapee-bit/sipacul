...
2. membangun target migration, API runtime, dan frontend dengan tag unik;
3. memulai stack Compose dengan project, volume, network, dan port loopback
   yang unik;
4. menunggu PostgreSQL, migration, API, dan frontend mencapai state yang
   diwajibkan;
5. memeriksa migration terakhir, route `/login`, proxy bootstrap, publikasi
   port, serta keanggotaan network setiap service;
...
- PostgreSQL, migrator, dan API tidak memublikasikan port host.
- Frontend hanya diikat ke `127.0.0.1` pada port acak.
...
- Container produksi dengan prefix `sipacul-production-` diaudit sebelum dan
  sesudah gate.
- Build cache dan base image Docker dapat dipakai ulang, tetapi tiga tag image
...
- memilih penyedia hosting, domain, reverse proxy, atau TLS;
